...
class Nif {
 public:
  ...
  friend std::ifstream& operator>>(std::ifstream& input, Nif& nif);
...
// Sobrecarga de operador de extracción de archivo
std::ifstream& operator>>(std::ifstream& input, Nif& nif) {
  long number;
  input >> number;
  nif = Nif(number);
  return input;
...
std::istream& operator>>(std::istream& input, Nif& nif) {
  long number;
  input >> number;
  nif = Nif(number);
  return input;
